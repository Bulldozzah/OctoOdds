...
async function cached<T>(
  ...
  // A forced read skips the stored value — that is the whole point of it — but
  // still joins a request already in flight, which is live data by definition
  // and saves a double-click from spending two credits.
  ...
  if (pending) {
    ...
    // The owner writes the entry before we resume, so this is the real time of
    // the upstream call rather than the time this caller happened to ask.
    return { value, cached: true, fetchedAt: memo.get(key)?.fetchedAt ?? now };
  }
...
export async function assertApprovedUser(accessToken: string): Promise<void> {
  ...
  const { data: profile } = await client
    .from("profiles")
    .select("role,is_paid,subscription_expires_at")
    .eq("id", data.user.id)
    .single();

  // Admins and superusers never pay; a regular user needs an active paid
  // subscription. Mirrors hasPaidAccess() on the client.
  const role = profile?.role;
  const notExpired =
    !profile?.subscription_expires_at ||
    new Date(profile.subscription_expires_at).getTime() > Date.now();
  const hasAccess =
    role === "admin" || role === "superuser" || (Boolean(profile?.is_paid) && notExpired);
  if (!hasAccess) throw new Error("Your subscription is not active — choose a plan to continue.");
}
...
export async function loadLeagueOdds(
  ...
  const {
    ...
    cached: fromCache,
    fetchedAt,
  } = await cached<Omit<OddsResponse, never>>(
    path,
    ODDS_TTL_MS,
    async () => {
