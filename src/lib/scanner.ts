...
// One letter per leg, so this must cover every combo size the Scanner offers
// (currently 1–4). A short array does not fail loudly: comboScenarios would
// name the extra legs "undefinedW", and that name is the ordering contract
// every downstream reader indexes off — pattern keys, exclusion text, and the
// rows persisted onto a saved bet.
