...
export function ExposureWarning({
  ...
  return (
    <div
      role="alert"
      className={cn(
        "flex gap-3 rounded-xl border border-destructive/40 bg-destructive/10 text-destructive",
        compact ? "px-3 py-2 text-xs" : "px-4 py-3 text-sm",
      )}
    >
      <AlertTriangle className={cn("mt-0.5 shrink-0", compact ? "size-3.5" : "size-4")} />
